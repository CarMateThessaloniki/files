...
<?php if ( !is_user_logged_in() ) : ?>
	...
	<div id="message" class="bp-template-notice">
		<p>You need to <a data-reveal-id="login_panel" href="<?php echo site_url( 'wp-login.php' ) ?>">log in</a> <?php if ( bp_get_signup_allowed() ) : ?> or <?php printf( __( ' <a data-reveal-id="register_panel" class="create-account" href="%s" title="Create an account">create an account</a>', 'buddypress' ), site_url( BP_REGISTER_SLUG . '/' ) ) ?><?php endif; ?> to post to this user's Wall.</p>
	</div>
	...
	<div id="whats-new-content">
		...
		<div id="whats-new-options">
			<div id="whats-new-submit">
				<input type="submit" class="button radius small" name="aw-whats-new-submit" id="aw-whats-new-submit" value="<?php _e( 'Post Update', 'buddypress' ); ?>" />
			</div>
