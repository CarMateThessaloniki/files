...
add_action("gform_after_submission_2", "after_submission_caroffer", 10, 2);
function after_submission_caroffer($entry, $form){
    $post_id = $entry['post_id'];
    $post = get_post_meta($post_id);
    $start = $post['carmate_start'][0];
    $dest = $post['carmate_destination'][0];
    $updated_caroffer = array(
	'ID' => $entry['post_id'],
	'post_title' => $start." - ".$dest
    );
    wp_update_post($updated_caroffer);
}
