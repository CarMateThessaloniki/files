...
require ( dirname( __FILE__ ) . '/utils/class.version.check.php' );
	...
	if ( is_admin() ) { // Only display this message in the admin area
		...
		echo '<div class="error fade"><p>'.$message.'</p></div>';
			...
			if (is_child_theme() && file_exists(get_stylesheet_directory() . '/languages/bp-album-' . get_locale() . '.mo')) {
				load_textdomain('bp-album', get_stylesheet_directory() . '/languages/bp-album-' . get_locale() . '.mo');
			} else {
