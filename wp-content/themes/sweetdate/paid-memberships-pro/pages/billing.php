<?php 				
	...
	{ 
	?>
		<p><?php printf(__("Logged in as <strong>%s</strong>.", "pmpro"), $current_user->user_login);?> <small><a href="<?php echo wp_logout_url(get_bloginfo("url") . "/membership-checkout/?level=" . $level->id);?>" class="small radius button bordered"><i class="icon  icon-off"></i> <?php _e("logout", "pmpro");?></a></small></p>
		<ul class="no-bullet">
			<li><strong><?php _e("Level", "pmpro");?>:</strong> <span class="label radius"><?php echo $level->name?></span></li>
...
<?php if(pmpro_isLevelRecurring($level)) { ?>
	...
	<?php } ?>
<?php } else { ?>
	<p class="additional-info"><?php _e("This subscription is not recurring. So you don't need to update your billing information.", "pmpro");?></p>
<?php } ?>