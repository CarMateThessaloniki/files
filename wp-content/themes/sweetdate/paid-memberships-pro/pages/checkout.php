...
<form id="pmpro_form" class="pmpro_form custom" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<tbody>                
		<tr>
			<td>				
				<p>
					<?php printf(__('You have selected the <strong class="label radius">%s</strong> membership level.', 'kleo_framework'), $pmpro_level->name);?>
				</p>
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>                        	                       										
		...
		<p>
			<?php printf(__('You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s" class="link">log out now</a>.', 'kleo_framework'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>			
		</p>
	<?php } ?>
...
<?php echo wpautop($tospage->post_content)?>
					...
					<label for="tos"><input type="checkbox" name="tos" value="1" id="tos" /> <?php printf(__('I agree to the %s', 'pmpro'), $tospage->post_title);?></label>
				</td>
			</tr>
		</tbody>
		</table>
		...
		jQuery(document).ready(function () {
			jQuery("#pmpro_form").submit(function() {
				if(!jQuery("#tos").is(":checked")) {
					alert("<?php _e('You must agree to the terms and conditions', 'kleo_framework'); ?>");
					jQuery('input[type=submit]', this).attr('disabled', false);
					jQuery('input[type=image]', this).attr('disabled', false);
